...
```
runs/<run_id>/
  train.*                # required: self-contained, seeded training script that
                         #           reproduces this model end-to-end
  infer.*                # required: scores unseen data with the shipped model
                         #           (convention: python infer.py <in.csv> <out.csv>)
  model.*                # required: serialized trained model (joblib/pkl/...)
  predictions.parquet    # required: row_id (0-based row index into the goal dataset),
                         #           y_true, y_pred, (classification) proba_*
  predictions.csv        # auto-generated by run_finish from the parquet
  ...
  meta.json              # required: model description, hyperparams, feature list,
                         #           training time, random seed; recommended:
                         #           feature_importance
```

`run_finish` records the sha256 of `train.*` and `infer.*` in the ledger, so every
result is traceable to the exact code that produced it.
