...
def _dictionary_section(context: list[dict] | None) -> str:
    if not context:
        return ""
    rows = "".join(
        f"<tr><td>{_esc(c['feature'])}</td><td>{_esc(c['meaning'])}</td>"
        f"<td>{_esc(c['source'] or '—')}</td></tr>"
        for c in context
    )
    return (
        "<h2>Data dictionary</h2><div class='card'>"
        "<table><tr><th>feature</th><th>meaning</th><th>source</th></tr>"
        f"{rows}</table></div>"
    )
...
def verdict_report_html(
    ...
    parts.append(_dictionary_section(context))
    parts.append(
...
def render_verdict_report(
    goal_summary: dict, forensics_results: dict, verdict: dict, charts_dir: Path | str,
    out_path: Path | str, context: list[dict] | None = None,
) -> Path:
    html_text = verdict_report_html(goal_summary, forensics_results, verdict, charts_dir, context=context)
